...
class Graph:
    def __init__(self, nodes, saturation, hamiltonian=True):
    ...
    def create_non_hamilton_graph(self):
        ...
        hamiltonian_nodes = self.nodes[:-1]
        isolated_node = self.nodes[-1]

        # Create a Hamiltonian cycle for n-1 nodes
        cycle = hamiltonian_nodes[:]
        random.shuffle(cycle)
